...
const rightWall = Bodies.rectangle(CANVAS_WIDTH + 1, CANVAS_HEIGHT / 2 + BALL_SIZE * 2, 1, CANVAS_HEIGHT, {
  isStatic: true,
});
...
Events.on(engine, "collisionActive", ({ pairs }) => {
  const filteredPairs = pairs.forEach((pair) => {
    if (
      ...
      onScoreUpdate(Math.round(pair.bodyB.dropX), pair.bodyB.restitution, pair.bodyB.size, bucketId + 1);
