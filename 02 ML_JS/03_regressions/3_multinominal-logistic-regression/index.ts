...
let {
  ...
  testFeatures,
  testLabels,
}: { features: number[][]; labels: number[][][]; testFeatures: number[][]; testLabels: number[][][] } = loadCSV(
  "./cars.csv",
  {
    shuffle: true,
...
const result = multinominalLogisticRegression.test(testFeatures, _.flatMap(testLabels));
